...
class HtmltopdfValidationException implements Exception {
  final List<String> errors;

  HtmltopdfValidationException(this.errors);

  @override
  String toString() => 'HtmltopdfValidationException: ${errors.join("; ")}';
}
...
/// HTML to PDF API client.
///
/// For more information, visit: https://htmltopdf.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [html] (required) - The HTML content to convert to PDF. Ensure that the HTML is escaped properly
/// * [marginTop] - The top margin of the PDF in inches (e.g., 0.5)
/// * [marginRight] - The right margin of the PDF in inches (e.g., 0.5)
/// * [marginBottom] - The bottom margin of the PDF in inches (e.g., 0.5)
/// * [marginLeft] - The left margin of the PDF in inches (e.g., 0.5)
...
class HtmltopdfClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'marginTop': ValidationRule(type: 'number', required: false),
    'marginRight': ValidationRule(type: 'number', required: false),
    'marginBottom': ValidationRule(type: 'number', required: false),
    'marginLeft': ValidationRule(type: 'number', required: false),
    'landscape': ValidationRule(type: 'boolean', required: false),
  };

  HtmltopdfClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/htmltopdf',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HtmltopdfValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HtmltopdfValidationException(errors);
    }
  }

  /// Execute a request to the HTML to PDF API.
  ///
  /// Parameters are validated before sending the request.
  Future<HtmltopdfResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HtmltopdfException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HtmltopdfResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HtmltopdfException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HtmltopdfException('Resource not found');
      } else {
        throw HtmltopdfException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HtmltopdfException) rethrow;
      throw HtmltopdfException('Request failed: $e');
    }
  }
...
/// Exception thrown by the HTML to PDF API client.
