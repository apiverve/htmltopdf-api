/**
 * HTML to PDF API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the HTML to PDF API.
 ...
 */
async function callHTMLtoPDFAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callHTMLtoPDFAPIWithErrorHandling({
