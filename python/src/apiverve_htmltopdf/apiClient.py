...
class HtmltopdfAPIClientError(Exception):
...
class ValidationError(HtmltopdfAPIClientError):
...
class HtmltopdfAPIClient:
    ...
    VALIDATION_RULES = {"html": {"type": "string", "required": True}, "marginTop": {"type": "number", "required": False, "default": "0.4"}, "marginRight": {"type": "number", "required": False, "default": "0.4"}, "marginBottom": {"type": "number", "required": False, "default": "0.4"}, "marginLeft": {"type": "number", "required": False, "default": "0.4"}, "landscape": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HtmltopdfAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HtmltopdfAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HtmltopdfAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns HTMLtoPDFData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises HtmltopdfAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HtmltopdfAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HtmltopdfAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HtmltopdfAPIClientError(f"Request failed: {str(e)}")
